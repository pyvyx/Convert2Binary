...
int main(int argc, char **argv)
{
    ...
    if(argc < 2 || argc > 6)
    {
        std::cout << "Usage: c2b <filename> <options>" << std::endl;
        return -1;
    }
    else if(argc == 2 && (std::string(argv[1]) == "--help" || std::string(argv[1]) == "-h"))
    {
        ...
        std::cout << "          -c          : for console output" << "\n";
        std::cout << "          -n <number> : to change the number of byte blocks per line" << "\n";
        std::cout << "          -f          : can be faster but will use more memory" << "\n";
        std::cout << "  All three options can be combined, but if they are combined the" << "\n";
        std::cout << "  -c options has to be the first, -n the second and -f at last" << "\n";
        return -1;
    }
    else if(argc > 2 && (std::string(argv[2]) == "-c" || std::string(argv[2]) == "-C"))
        console_output = true;

    else if(argc > 2 && (std::string(argv[2]) != "-f" && std::string(argv[2]) != "-F" && std::string(argv[2]) != "-n" && std::string(argv[2]) != "-n"))
    ...
    if(argc == 3 && (std::string(argv[2]) == "-f" || std::string(argv[2]) == "-F"))
        use_strings = true;
    else if(argc == 4 && (std::string(argv[3]) == "-f" || std::string(argv[3]) == "-F"))
        use_strings = true;
    else if(argc == 5 && (std::string(argv[4]) == "-f" || std::string(argv[4]) == "-F"))
        use_strings = true;
    else if(argc == 6 && (std::string(argv[5]) == "-f" || std::string(argv[5]) == "-F"))
        use_strings = true;
    ...
    for(int j = 0; j < lines.size(); ++j)
    {
        ...
        {
            if(use_strings)
                out += "    " + DecToHex(line_counter_T,8) + ":  " + lines[j];
        ...
        for(std::size_t i = 0; i < lines[j].size(); ++i)
        {
            ...
            if(use_strings)
                out += DecToHex(passed_bytes,8) + ":  " + bitToString(bit_vec) + charToString(char_vec) + "\n";
            else if(!console_output)
